...
export default function EmployeeMessagesPage() {
  ...
  const messagesQuery = useApiQuery<Message[]>(["employee", "messages"], "/employee/messages", { fallback: [] });

  const sendMessage = useApiMutation<{ content: string }>(
    async (payload) => (await api.post("/employee/messages", payload)).data,
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] flex-col space-y-6">
      <PageHeader eyebrow="My Workspace" title="Messages" description="Chat with your team and managers." />
